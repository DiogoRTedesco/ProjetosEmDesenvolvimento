...
import { PDFDocument } from '../../../PDF/SaidasPDF';
import { api } from '../../../hooks/useApi';

export const RelatorioSaida = () => {
    ...
    const buscarRegistroSaida = async () => {
        await api.post('/relatorioSaidas', {
            entrada: entrada,
            saida: saida,
            status: status
        }
    ...
    return (
        <div className='Rel'>
            <Form >
                ...
                <Button onClick={buscarRegistroSaida}>Buscar</Button>
            </Form>

            <div style={{display:  show === false ? 'none': 'block'}}>
                <h1>Relatório de Saídas : Período : {entrada.replace(/-/g, "/")} - {saida.replace(/-/g, "/")}</h1>
